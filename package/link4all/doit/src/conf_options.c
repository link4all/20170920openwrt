...
static int conf_udp_server(const char* value) {
        strcpy(conf.udp_server, value);
        return 1;
}
...
static int conf_udp_port(const char* value) {
        conf.udp_port = atoi(value);
        return 1;
}
...
static int conf_upload_inter(const char* value) {
        conf.upload_inter = atoi(value) ;
        return 1;
}


static int conf_filter_type(const char* value) {
        conf.filter_type = atoi(value) ;
        return 1;
}
...
static struct conf_option conf_options[] = {
	...
	{ 's', "channel_scan",		1, "0",		conf_channel_scan },
	{  0 , "channel_dwell",		1, "200", 	conf_channel_dwell },
	...
	{ 'U', "udp server",		1, NULL,	conf_udp_server },
	{ 'P', "udp port",		1, NULL,	conf_udp_port },
	{ 'I', "upload inter",		1, NULL,	conf_upload_inter },
	{ 'F', "filter type",		1, NULL,	conf_filter_type },
};
...
void print_usage(const char* name) {
	printf("\nUsage: %s [-h] [-q] [-D] [-c file] [-i interface] [-t sec] [-d ms] [-V view] [-b bytes]\n"
		...
		"  -X[filename]\tAllow control socket on 'filename' (/tmp/doit)\n"
