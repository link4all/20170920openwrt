...
sprintf(send_buf,"%s|%s|%03d|%02x|%s|%d",mac_buf,p_buf,p->phy_signal,p->wlan_type,buf,p->wlan_channel);
if(p->wlan_type==0x80){
...
sprintf(send_buf,"%s|%s|%03d|%02x|%s|%d|%s|%s",mac_buf,p_buf,p->phy_signal,p->wlan_type,buf,p->wlan_channel,jb,p->wlan_essid);
