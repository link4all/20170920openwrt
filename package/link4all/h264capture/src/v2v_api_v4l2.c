...
int v2v_module_v4l2_get_fream(FreamBuffer *freambuf_t)
{
	struct timeval tv;
	struct v4l2_buffer buf;
	fd_set fds; 
...
{	
	int ret = 0;
	pthread_t tid;
	int fp = -1,fp1,fp2,fp3;
	FreamBuffer freambuf_t;
	int i= 0,j = 0;

	printf("************************************\n");
	if(argc<2)
		{
			printf("you better add filename\n ");
			printf("you save file is %s\n",CAPTURE_FILE);
			argv[1] = CAPTURE_FILE;
	...
	ret = v2v_module_v4l2_device_init();
	if(ret != 0)
		exit_error("init  \n");	
	...
	//while(1)
		{
			memset (&freambuf_t,0,sizeof(struct Fream_Buffer));
			
			ret = v2v_module_v4l2_start_capturing();
		//	sleep(5);
			if(ret != 0)
				exit_error("starting capturing \n");
			while(1)
				{
					fp = open(argv[1], O_WRONLY |O_CREAT |O_TRUNC|O_APPEND, 0777);
					printf("the numberis %d********************** \n",j++);
				while(1)
				{
					ret= v2v_module_v4l2_get_fream(&freambuf_t);
					if(ret ==0)
						;
				//	printf("is ok\n");
					printf(" THE count is  %d  :%d    OK **********%d\n",j,i++,freambuf_t.length);
					if(ret == 1)
					continue;
				
					if(write(fp,freambuf_t.buf,freambuf_t.length)<0)
						{
							printf("error\n");
							exit(-1);
						}
			...
			ret = v2v_module_v4l2_stop_capturing();
			if(ret != 0)
				exit_error("stop catpturing \n");
		}
	printf("input any char end the procedure\n ");
	getchar();
	
	ret = v2v_module_v4l2_device_uinit();
	if(ret != 0)
		exit_error("uinit");
