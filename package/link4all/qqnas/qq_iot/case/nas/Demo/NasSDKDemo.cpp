...
int notify_file_download_complete(unsigned long long from_client, const char* tmp_file_path, int file_type, const char* specified_dst_dir)
{
	...
	if(specified_dst_dir == NULL){
		switch(file_type)
		{
   			case  file_type_pic: snprintf(dst_dir,512,"/mnt/mmcblk0p1/pic");
                              break;
   			case  file_type_audio: snprintf(dst_dir,512,"/mnt/mmcblk0p1/audio");
                              break;
 			case  file_type_video: snprintf(dst_dir,512,"/mnt/mmcblk0p1/video");
                              break;
   			case  file_type_doc: snprintf(dst_dir,512,"/mnt/mmcblk0p1/doc");
                              break;
   			case  file_type_other: snprintf(dst_dir,512,"/mnt/mmcblk0p1/other");
                              break;

      			default: snprintf(dst_dir,512,"/mnt/mmcblk0p1/other");
                              break;
		}
	}else{
		snprintf(dst_dir,512,"%s/%s",g_root,specified_dst_dir);           //存放到specified_dst_dir目录下. specified_dst_dir为相对路径，需转换为绝对路径
...
void log_func(int level, const char* module, int line, const char* message)
{
	...
	FILE * file = fopen("/mnt/mmcblk0p1/sys/devicelog", "aw+");
	    if (file)
	    {
	    	fprintf(file, "%s\t%d\t%s\n", module, line, message);
	        fclose(file);
...
bool initdevice()
{
	...
	//if (0 != readBufferFromFile("./licence.sign.file.txt", license, sizeof(license), &nLicenseSize))
	  if (0 != readBufferFromFile("/etc/qqiot/save/licence.sign.file.txt", license, sizeof(license), &nLicenseSize))
	{
	...
	if(0 != readBufferFromFile("/etc/qqiot/save/GUID_file.txt", guid, sizeof(guid), &nGUIDSize))
	{
	...
	int nPubkeySize = 0;
    //if (0 != readBufferFromFile("./1700004239.pem", svrPubkey, sizeof(svrPubkey), &nPubkeySize))
    if (0 != readBufferFromFile("/etc/qqiot/save/1700004239.pem", svrPubkey, sizeof(svrPubkey), &nPubkeySize))
	{
	...
	init_path.system_path = "/mnt/mmcblk0p1/sys/";
	init_path.system_path_capicity  = 10240;
	//init_path.app_path = "./";
	init_path.app_path = "/mnt/mmcblk0p1/sys/";
	init_path.app_path_capicity  = 1024000;
	init_path.temp_path = "./";
	init_path.temp_path = "/mnt/mmcblk0p1/sys/";
	...
	ret = tx_nas_init_file_transfer(&file_transfer_nofity, "/mnt/mmcblk0p1/sys/QQ_Recv/");
	if (0 == ret) {
