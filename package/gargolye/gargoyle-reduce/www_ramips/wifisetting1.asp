            ...
            <div class="wrap-form">
                <form class="form-info" id="form0" >
                    <label>
                        <div class="name"></div>
                        <div>
                            <input type="checkbox" value="1" name="enable" <%= `uci get wireless.mt7628.disable >/dev/null 2>&1  || echo checked` %>/>开启无线
                        </div>
                    </label>
                      <label>
                                ...
                                <div>
                                    ...
                                    <input type="checkbox" value="1" name="hidssid" <%= `uci get wireless.ap.hidden >/dev/null 2>&1  && echo checked` %>/>隐藏SSID
                                </div>
                            </label>
                            <label>
                                ...
                                <div>
                                    <select name="etype"  >
                                        <option value="none" <% [ `uci get wireless.ap.encryption |grep psk` ] && echo 'selected="true"' %> >不加密</option>
                                        <option value="psk2" <% [ `uci get wireless.ap.encryption |grep psk` ] && echo 'selected="true"' %> >加密</option>
                                    </select>
                                </div>
                            </label>
                            <label>
                    ...
                    <label>
                        ...
                        <div>
                            <select name="txpower" id="stong">
                                <option value="100" <% [ `uci get wireless.mt7628.txpower |grep 100` ] && echo 'selected="true"' %> >高</option>
                                <option value="60" <% [ `uci get wireless.mt7628.txpower |grep 60` ] && echo 'selected="true"' %> >中</option>
                                <option value="40" <% [ `uci get wireless.mt7628.txpower |grep 40` ] && echo 'selected="true"' %> >低</option>
                            </select>
                        </div>
                    </label>
                </form>
