...
EOF
	...
	# curl -d 'app=vlan&action=lan_port_status' -L "http://10.10.11.254"	# 获取端口状态
	printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
...
for P in $PORTS; do
	...
	# echo "{\"Port\":${counter},\"Lan\":\"$PORT\",\"Status\":\"$STATUS\"}"
...
echo "$RESULT"
	...
	# curl -d 'app=vlan&action=port_status&switch=switch0' -L "http://10.10.11.254"	# 获取端口状态
	printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
printf '{'
