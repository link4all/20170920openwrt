...
# action: notice_get_page 获取单页消息内容
...
# action: notice_get_page 获取单条消息detail
	...
	eval $(/usr/shellgui/progs/main.sbin l_p_n $COOKIE_lang)
		...
		# result=$(echo "$result" | jshon -e "result" -e 0 \
		# -s "${desc}" -i "Desc" \
		# -s "${detail}" -i "Detail" \
		# -d "Variable" \
		# -p -p -j)
...
# action: email_extra_setting Email副设置
