...
if [ "$FORM_ip_source" = "web" ]; then
uci set ddns.${config}.ip_source='web'
uci set ddns.${config}.ip_network="$FORM_wan_zone"
else
	uci batch <<EOF
set ddns.${config}.ip_source='network'
set ddns.${config}.ip_network="$FORM_wan_zone"
set ddns.${config}.interface="$FORM_wan_zone"
EOF
fi
	uci commit ddns
	printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
