...
define_wan_if()
{
if [ $(uci get mwan3.default.enabled) -gt 0 ]; then
	network_str=$(uci -P /var/state show network)
	ifces=$(uci show mwan3 | grep '=interface' | cut -d  '=' -f1 | cut -d '.' -f2)
	for ifce in $ifces; do
		echo "${network_str}" | grep -qE 'network.'"${ifce}"'=interface' && wans="$wans ${ifce}"
	done
else
	wans="wan"
fi
for wan_if in ${wans:-wan}; do
	wan_ifs="$wan_ifs $(uci -P /var/state get network.${wan_if}.ifname 2>/dev/null || uci -P /var/state get network.${wan_if}.device)"
done
}
...
create_l7marker_chain()
{
	...
	qos_active=$(ls /etc/rc.d/*qos_shellgui* 2>/dev/null)
	if [ -n "$qos_active" ] ; then
		qos_l7=$(uci show qos_shellgui | sed '/layer7=/!d; s/^.*=//g')
	fi
...
insert_pf_loopback_rules()
{
	...
	dcft "nat"    "pf_loopback_A"
	dcft "filter" "pf_loopback_B"
	dcft "nat"    "pf_loopback_C"

	define_wan_if
	if [ -z "$wan_ifs" ]  ; then return ; fi
...
{
	local UCI_CONFIG_DIR="/usr/shellgui/shellguilighttpd/www/apps/restriction"
	define_wan_if
	if [ -z "$wan_ifs" ]  ; then return ; fi
	...
	if [ -n "$egress_exists" ] ; then
		dcft filter egress_whitelist
		dcft filter egress_restrictions
	fi
	if [ -n "$ingress_exists" ] ; then
		dcft filter ingress_whitelist
		dcft filter ingress_restrictions
	fi
...
for wan_if in $wan_ifs; do
	...
	package_name="restriction_uci"
	parse_rule_config()
	{
		section=$1
		section_type=$(uci -c/usr/shellgui/shellguilighttpd/www/apps/restriction get "$package_name"."$section")
		...
		if [ "$enabled" = "1" ] && ( [ "$section_type"  = "restriction_rule" ] || [ "$section_type" = "whitelist_rule" ] ) ; then
			...
			if [ -n "$app_proto" ] ; then
				...
				uci -c/usr/shellgui/shellguilighttpd/www/apps/restriction set "$package_name"."$section".connmark="$app_proto_connmark/$app_proto_mask"
			fi
			if [ -n "$not_app_proto" ] ; then
				...
				uci -c/usr/shellgui/shellguilighttpd/www/apps/restriction set "$package_name"."$section".not_connmark="$not_app_proto_connmark/$not_app_proto_mask"
			fi
			...
			mir -d "/usr/shellgui/shellguilighttpd/www/apps/restriction" -p "$package_name" -s "$section" -t "$table" -c "$chain" -g "$target" $ingress
			mir -d "/usr/shellgui/shellguilighttpd/www/apps/restriction" -p "$package_name" -s "$section" -t "$table" -c "$chain" -g "$target" $ingress -r

			uci -c/usr/shellgui/shellguilighttpd/www/apps/restriction del "$package_name"."$section".connmark 2>/dev/null	
			uci -c/usr/shellgui/shellguilighttpd/www/apps/restriction del "$package_name"."$section".not_connmark	 2>/dev/null
		fi
	}
...
initialize_quotas()
{
	...
	full_qos_enabled=$(ls /etc/rc.d/*qos_shellgui 2>/dev/null)

	if [ -n "$full_qos_enabled" ] ; then
		full_up=$(uci get qos_shellgui.upload.total_bandwidth 2>/dev/null)
		full_down=$(uci get qos_shellgui.download.total_bandwidth 2>/dev/null)
	...
	# r_quotas does the hard work of building quota chains & rebuilding crontab file to do backups
	#
	...
	if [ -z "$full_qos_enabled" ] ; then
		rm -f /usr/shellgui/shellguilighttpd/www/apps/quotas/qos_class_enabled
	fi
	rm -f /tmp/state/quotas_uci
	cp /usr/shellgui/shellguilighttpd/www/apps/quotas/quotas_uci /tmp/state/quotas_uci
	wan_if_main=$(uci -P /var/state get network.wan.ifname 2>/dev/null || uci -P /var/state get network.wan.device)
	if [ ! -f /usr/shellgui/shellguilighttpd/www/apps/quotas/qos_class_enabled ] ; then

		for config in $(uci -c/tmp/state show quotas_uci | grep -E 'exceeded_.*_class_mark=' | cut -d '.' -f1-2 | sort -n | uniq); do
		uci -c/tmp/state del ${config}
		done &>/dev/null
		uci -c/tmp/state commit quotas_uci

		r_quotas    -w $wan_if_main -d $death_mark -m $death_mask -s "$lan_ip/$lan_mask" -c "0 0,4,8,12,16,20 * * * /usr/bin/b_quotas >/dev/null 2>&1"
		initialiaze_quota_qos
	else

		for config in $(uci -c/tmp/state show quotas_uci | grep -E 'exceeded_.*_speed=' | cut -d '.' -f1-2 | sort -n | uniq); do
		uci -c/tmp/state del ${config}
		done &>/dev/null
		uci -c/tmp/state commit quotas_uci

		r_quotas -q -w $wan_if_main -d $death_mark -m $death_mask -s "$lan_ip/$lan_mask" -c "0 0,4,8,12,16,20 * * * /usr/bin/b_quotas >/dev/null 2>&1"
		cleanup_old_quota_qos
	fi
	for wan_if in $wan_ifs; do
	[ "${wan_if}" = "$wan_if_main" ] && continue
	iptables -t mangle -I forward_quotas -o ${wan_if} -m connmark --mark 0x0/0x8000 -j CONNMARK --set-xmark 0xf000000/0xf000000
	iptables -t mangle -I forward_quotas -i ${wan_if} -m connmark --mark 0x0/0x8000 -j CONNMARK --set-xmark 0xf000000/0xf000000
	iptables -t mangle -I forward_quotas -i ${wan_if} -m connmark --mark 0x0/0x8000 -j ingress_quotas
	iptables -t mangle -I forward_quotas -o ${wan_if} -m connmark --mark 0x0/0x8000 -j egress_quotas
	iptables -t mangle -I OUTPUT -o ${wan_if} -m connmark --mark 0x0/0x8000 -j egress_quotas
	iptables -t mangle -I INPUT -i ${wan_if} -m connmark --mark 0x0/0x8000 -j combined_quotas
	iptables -t mangle -I OUTPUT -o ${wan_if} -m connmark --mark 0x0/0x8000 -j combined_quotas
	iptables -t mangle -I INPUT -i ${wan_if} -m connmark --mark 0x0/0x8000 -j ingress_quotas
	done
...
initialiaze_quota_qos()
{
	...
	if [ -n "$wan_ifs" ] && [ $num_up_bands -gt 1 ] && [ $num_down_bands -gt 1 ] ; then
...
block_static_ip_mismatches()
{
	...
	dcft static_mismatch_check filter
...
add_adsl_modem_routes()
{
	...
	for wan_if in $wan_ifs; do
		if echo ${wan_if} | grep -q pppoe; then
		# wan_dev=$(uci -q get network.wan.ifname) #not really the interface, but the device
		iptables -A postrouting_rule -t nat -o ${wan_if} -j MASQUERADE
		iptables -A forwarding_rule -o ${wan_if} -j ACCEPT
		/etc/ppp/ip-up.d/modemaccess.sh firewall ${wan_if}
		fi
	done
	# fi
}
