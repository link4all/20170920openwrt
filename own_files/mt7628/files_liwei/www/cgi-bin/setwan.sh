...
if [ "$FORM_wan_mode"x = "tolan"x ];then
uci -q set network.lan.ifname="eth0.1 eth0.2"
uci -q set network.wan.ifname=""
uci -q set network.wan.proto=''
uci -q set network.wan.metric=""
echo "{"
echo "\"mode\":\"LAN\""
echo "}"
else
uci -q set network.lan.ifname="eth0.1"
uci -q set network.wan.ifname="eth0.2"
uci -q set network.wan.metric=""
echo "{"
echo "\"mode\":\"WAN\""
echo "}"
