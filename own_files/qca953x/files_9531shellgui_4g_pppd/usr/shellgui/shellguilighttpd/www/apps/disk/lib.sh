...
echo "${result}" | jshon -e "${uuid}" &>/dev/null || continue
...
if [ "${FORM_action}" = "swap_mount" ] &>/dev/null; then
	eval $(/usr/shellgui/progs/main.sbin l_p '_LANG_Form_' ${FORM_app} $COOKIE_lang)
	printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
	cat <<EOF
{"status":0,"msg":"${_LANG_Form_Modify_success}","jump_url":"/?app=disk","seconds":"2000"}
EOF
	uci batch <<EOF &>/dev/null
add fstab swap
set fstab.@swap[-1].device='$FORM_part'
set fstab.@swap[-1].type='swap'
set fstab.@swap[-1].enabled='1'
EOF
uci commit fstab
block mount
	return
elif [ "${FORM_action}" = "swap_unmount" ] &>/dev/null; then
	eval $(/usr/shellgui/progs/main.sbin l_p '_LANG_Form_' ${FORM_app} $COOKIE_lang)
	printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
	cat <<EOF
{"status":0,"msg":"${_LANG_Form_Modify_success}","jump_url":"/?app=disk","seconds":"2000"}
EOF
	(cfg=$(uci show -X fstab | grep -E 'device=[[:punct:]]'"${FORM_part}"'[[:punct:]]' | cut -d '.' -f-2)
	if [ -n "$cfg" ]; then
	uci set "$cfg"=
	uci commit fstab
	block mount
	fi
	swapoff ${FORM_part}) &>/dev/null
	return
elif [ "${FORM_action}" = "remove_partition" ] &>/dev/null; then
	eval $(/usr/shellgui/progs/main.sbin l_p '_LANG_Form_' ${FORM_app} $COOKIE_lang)
	printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
	cat <<EOF
{"status":0,"msg":"${_LANG_Form_Modify_success}","jump_url":"/?app=disk","seconds":"2000"}
EOF
	num=$(echo $FORM_part | grep -Eo '[0-9]*$')
	dev=$(echo $FORM_part | sed 's/[0-9]*$//')
	(echo -e 'd\n'"$num"'\n\nw\n' | fdisk $dev
partprobe) &>/dev/null
elif [ "${FORM_action}" = "format_partition" ] &>/dev/null; then
	eval $(/usr/shellgui/progs/main.sbin l_p '_LANG_Form_' ${FORM_app} $COOKIE_lang)
	printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
	cat <<EOF
{"status":0,"msg":"${_LANG_Form_Modify_success}","jump_url":"/?app=disk","seconds":"2000"}
EOF
	(if [ "$FORM_type" = "swap" ]; then
	mkswap $FORM_part
	else
	echo y | mkfs.${FORM_type} $FORM_part
	fi) &>/dev/null
elif [ "${FORM_action}" = "add_new_partition" ] &>/dev/null; then
	eval $(/usr/shellgui/progs/main.sbin l_p '_LANG_Form_' ${FORM_app} $COOKIE_lang)
	printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
	cat <<EOF
{"status":0,"msg":"${_LANG_Form_Modify_success}","jump_url":"/?app=disk","seconds":"2000"}
...
partprobe) &>/dev/null
	return
elif [ "${FORM_action}" = "show_fstab" ] &>/dev/null; then
	printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
	uci_show_fstab
	return
