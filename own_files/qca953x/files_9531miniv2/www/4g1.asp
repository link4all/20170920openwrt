...
//    mode_change();
   if ($('input:radio[name="apnsel"]:checked').val() == 0){
    closeadv()
   }
   else{
    showadv()
            ...
            <div class="wrap-form">
             <form class="form-info" id="form0">
                    ...
                    <label class="">
                        ...
                        <div>
                            <input id="4gip" name="4gip" type="text" value="<% [ `ubus call network.interface.4g status |grep "\"address\":"|head -n 1 |cut -d: -f2 |tr -d "\"\, "` ] && ubus call network.interface.4g status |grep "\"address\":" |head -n 1|cut -d: -f2 |tr -d "\"\, " || echo "$g4_noip" %>" readonly="readonly" style="background-color:#eee" />
                        </div>
                    </label>
                    ...
                    <label class="">
                            ...
                            <select class="dialmode" name="dialmode" >
                    ...
                    <label> 
                        <div class="name"><%= $other_parameter%></div>
                        <input name="apnsel" type="radio" value="0" onclick="closeadv()" <% [ `uci get config4g.@4G[0].apnsel |grep "0"` ] && echo checked="checked"    %>/><%= $auto%> 
                        <input name="apnsel" type="radio" value="1" onclick="showadv()" <% [ `uci get config4g.@4G[0].apnsel |grep "1"` ] && echo checked="checked"   %> /><%= $manual%>
                    </label> 
				...
				<input class="apn" name="apn" type="text" value="<% uci get config4g.@4G[0].apn %>" placeholder="apn" />
                    ...
                    <label class="">
                        ...
                        <div>
                            <input id="dialnumber" name="dialnumber" type="text" value="<% uci get network.4g.dialnumber 2>/dev/null %>" placeholder="*99#" />
                        </div>
                    </label>
