                ...
                <div class="col-sm-offset-2 col-sm-6 text-left">
					...
					<fieldset id="set_dnscdn_form" <% grep -q 'server=/#/127.0.0.1' /usr/shellgui/shellguilighttpd/www/apps/dns-cdn/all.dnsmasqd || printf 'disabled' %>>
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" id="chinadns_ips" data-validate="vaIPs-vaLength" name="chinadns_ips" value="<% get_dns_cdn %>" placeholder="114.114.114.114,223.5.5.5">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" data-validate="vaIP-vaLength" id="dns_forwader_ip" name="dns_forwader_ip" value="<% uci get dns-forwarder.@dns-forwarder[0].dns_servers %>" placeholder="8.8.8.8">
...
<script>
  $('#switch_dnscdn').click(function() {
      ...
      if (status) {
          ...
          $('#dns_forwader_ip').trigger('blur');
          $('#chinadns_ips').trigger('blur');
      } else {
          // disableBtn();
