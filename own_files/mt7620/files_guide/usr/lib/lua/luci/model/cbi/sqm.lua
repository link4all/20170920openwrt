...
m = Map("sqm", translate("Smart Queue Management"),
	translate("With <abbr title=\"Smart Queue Management\">SQM</abbr>,You can make every client smoothly on the Internet early," .. 
		"also the setting can be base on network interface."))
...
for _, iface in ipairs(ifaces) do
	if iface ~= "lo" and string.find(iface,"ifb") ~= 1 and string.find(iface,"wds") ~= 1 and iface ~= "eth2" and iface ~= "eth2.1" then
		if(iface == "br-lan") then
			n:value(translate("LAN"))
		elseif iface == "eth2.2" then
			n:value(translate("WAN"))
		elseif iface == "ra0" then
			n:value(translate("Router WIFI"))
		elseif iface == "rtk0" or iface == "wlan0" then
			n:value(translate("USB WIFI"))
		elseif iface == "apcli0" then
			n:value(translate("Router Relay"))
		else
			n:value(iface)
		end
	end
